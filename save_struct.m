...
% Writes a tab-delimited table from the input structure and a header line
% based on the field names; fields are written in the order in which they
% appear in the structure
if nargin==0
    error('requires an input argument'); 
end
if nargin>4
    error('too many input arguments'); 
end
if nargin==4 
    fprintf('USE OF DEPRECATED FORMATFLAG\n'); 
end
...
if nargout>1 
    error('only a single output possible'); 
end
...
if slen>chunksize
    ...
    for st=1:chunksize:slen
        ...
        Si = reorder_struct(S,st:en); % See reorder_struct code in repository
    ...
    for f=1:nf
        ...
        if isempty(C)
            if nr==-1
                error('Problematic leftmost column'); 
            end
            C = repmat({'...'},nr,1);
        end
        ...
        if nr~=length(C)
            error('Field %s is a different length', fld{f}); 
        end
        % add column title
        if ~noheader 
            C = [fld{f}; C]; 
